...
namespace CityCompanyCard_base.Card.Command
{
    public class CommandSpecial : ICommandCard
    {
        ...
        public override bool PlayCommand(IEventObject ev)
        {
            ...
            {
                //获取生物
                new Selector_ResUnitFactory().getGetCanSpecailUnitFilter().startISeletor(ev, out IUnitCard[] outputCard);
                if (outputCard.Length > 0)
                {
                    IUnitCard card = outputCard[0];
                    //获取该卡的对应能力
                    ev.resCard = card;
                    ev.resKeyValus.TryAdd(EventObjectExtractKey.POWER_NAME, "特殊");
